    ...
    <div className="flex min-h-screen flex-col items-center justify-center p-4">
      <main className="my-auto mb-16 flex w-full max-w-6xl flex-col items-center justify-center rounded-lg border border-matrix-medGreen bg-matrix-black/80 p-8 shadow-lg">
        ...
        <div className="mb-8 w-full border-b border-matrix-medGreen">
          ...
          <p className="mb-4 font-shareTech text-xl text-matrix-medGreen">System Training & Knowledge Matrix</p>
        </div>

        {/* Education Section */}
        <div className="w-full space-y-8">
          {educationData.map((edu, index) => (
            <div key={index} className="rounded border border-matrix-medGreen bg-matrix-darkGreen/20 p-4">
              <div className="mb-2 flex items-start justify-between">
                <h3 className="font-shareTech text-xl text-matrix-neonGreen">
                  [{edu.node} NODE]: {edu.school}
                </h3>
                <span className="font-firaCode text-sm text-matrix-medGreen">{edu.status || edu.years}</span>
              </div>
              <p className="mb-2 font-roboto text-sm text-matrix-medGreen">
                Program Matrix: {edu.program} | {edu.location}
              </p>
              <p className="text-justify font-shareTech text-sm leading-relaxed text-white">{edu.description}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 w-full border-t border-matrix-medGreen pt-4">
          <p className="text-center font-firaCode text-sm text-matrix-medGreen">[END OF EDUCATIONAL MATRIX]</p>
        </div>
      </main>
    </div>
  );
